...
// 게임 오브젝트 목록
static const std::string PlayerGo1 = "Player1";
static const std::string PlayerGo2 = "Player2";
static const std::string PlayerGo3 = "Player3";
...
void SoftRenderer::LoadScene3D()
{
	...
	// 플레이어 설정
	GameObject& goPlayer1 = g.CreateNewGameObject(PlayerGo1);
	goPlayer1.SetMesh(GameEngine::CubeMesh);
	goPlayer1.GetTransform().SetPosition(Vector3::Zero);
	goPlayer1.GetTransform().SetScale(Vector3::One * playerScale);
	goPlayer1.GetTransform().SetRotation(Rotator(0.f, 45.f, -60.f));
	goPlayer1.SetColor(LinearColor::Blue);

	std::random_device rd;
	std::mt19937 gen(rd());

	std::uniform_int_distribution<int> pos(-400, 400);
	std::normal_distribution<double> rot(0, 359);

	GameObject& goPlayer2 = g.CreateNewGameObject(PlayerGo2);
	goPlayer2.SetMesh(GameEngine::CubeMesh);
	goPlayer2.GetTransform().SetPosition(Vector3(pos(gen), pos(gen), pos(gen)));
	goPlayer2.GetTransform().SetScale(Vector3::One * playerScale);
	goPlayer2.GetTransform().SetRotation(Rotator(rot(gen), rot(gen), rot(gen)));
	goPlayer2.SetColor(LinearColor::Red);

	GameObject& goPlayer3 = g.CreateNewGameObject(PlayerGo3);
	goPlayer3.SetMesh(GameEngine::CubeMesh);
	goPlayer3.GetTransform().SetPosition(Vector3(pos(gen), pos(gen), pos(gen)));
	goPlayer3.GetTransform().SetScale(Vector3::One * playerScale);
	goPlayer3.GetTransform().SetRotation(Rotator(rot(gen), rot(gen), rot(gen)));
	goPlayer3.SetColor(LinearColor::Green);
...
void SoftRenderer::Update3D(float InDeltaSeconds)
{
	...
	GameObject& goPlayer1 = g.GetGameObject(PlayerGo1);
	GameObject& goPlayer2 = g.GetGameObject(PlayerGo2);
	GameObject& goPlayer3 = g.GetGameObject(PlayerGo3);
	CameraObject& camera = g.GetMainCamera();

	// 입력에 따른 플레이어 트랜스폼의 변경
	goPlayer1.GetTransform().AddPosition(Vector3::UnitZ * input.GetAxis(InputAxis::ZAxis) * moveSpeed * InDeltaSeconds);
	goPlayer1.GetTransform().AddPitchRotation(-input.GetAxis(InputAxis::WAxis) * rotateSpeed * InDeltaSeconds);
	goPlayer2.GetTransform().AddPosition(Vector3::UnitZ * input.GetAxis(InputAxis::ZAxis) * moveSpeed * InDeltaSeconds);
	goPlayer2.GetTransform().AddPitchRotation(-input.GetAxis(InputAxis::WAxis) * rotateSpeed * InDeltaSeconds);
	goPlayer3.GetTransform().AddPosition(Vector3::UnitZ * input.GetAxis(InputAxis::ZAxis) * moveSpeed * InDeltaSeconds);
	goPlayer3.GetTransform().AddPitchRotation(-input.GetAxis(InputAxis::WAxis) * rotateSpeed * InDeltaSeconds);
